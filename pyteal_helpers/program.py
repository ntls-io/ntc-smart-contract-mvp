...
def event(
    ...
    return Cond(
        [Txn.application_id() == Int(0), init], #if equal to zero then we have not assigned an applicatin ID and we know we are in the initialise phase
        ...
        [Txn.on_completion() == OnComplete.NoOp, no_op],#thats where we are allowed to define our own behavoiru instead of the above.
    )
